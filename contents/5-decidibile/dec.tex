...
Si può dimostrare con il teorema di fattorizzazione delle applicazioni
che:
...
\subsection{Tableaux rivisitato per KT, KB}
...
\item Regole di necessitazione\\
\def\arraystretch{3} 
\tabcolsep=10.8mm  \begin{tabular}{*2{c}}   $\dfrac{\sigma\,\boa}{\sigma_{n}\, a}$ & $\dfrac{\sigma\,\neg\dia}{\sigma_{n}\,\neg a}$
-- con $\sigma_{n}$ già presente nei nodi precedenti, oppure $\sigma_{n}$
\textbf{nodo corrente}\end{tabular}
\end{itemize}
es. Si provi che la formula: $(\boa\implies a)$ è un teorema in KT
...
\end{itemize}
\def\arraystretch{2} 
\tabcolsep=10.8mm  \begin{tabular}{{c}}  $\dfrac{\sigma\,\boa}{\sigma_{n}\, a}$ \\ $\dfrac{\sigma\,\neg\dia}{\sigma_{n}\,\neg a}$ 
